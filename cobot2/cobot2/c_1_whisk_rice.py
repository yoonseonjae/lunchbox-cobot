...
def perform_task():
    ...
    while True:       
        ...
        for name, pos in pos_list.items():
            ...
            if name not in  [ "scoop_5_l", "scoop_6_l", "periodic", "place_pre_l"]:
            ...
            if name in ["above_l"]:
                grip()
                wait(2)  # 각 위치에서 1초 대기
            if name in ["place_pre_l"]:
                movec(pos_list["scoop_6_l"], pos_list["place_pre_l"], vel=VELOCITY, acc=ACC, angle=[1,1])
